      ...
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-8">
          {["anillos", "collares", "aros"].map((item) => (
            <button
              ...
              className="font-light text-sm uppercase tracking-wider relative group transition-all duration-300 text-gray-800 hover:text-purple-800"
            >
              {item.charAt(0).toUpperCase() + item.slice(1)}
              <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-purple-600 group-hover:w-full transition-all duration-300"></span>
            </button>
          ))}
        </nav>
        ...
        <div className="flex items-center space-x-4">
          ...
          <button className="relative transition-transform duration-300 hover:scale-110" onClick={() => setCartOpen(true)}>
            ...
            {cart.length > 0 && (
              <span className="absolute -top-2 -right-2 bg-purple-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-medium animate-pulse">
                {cart.length}
              </span>
            )}
          </button>
        ...
        <nav className="flex flex-col items-center py-2">
          {["anillos", "collares", "aros"].map((item) => (
            <button
              ...
              className="w-full text-center text-gray-800 uppercase font-light tracking-wider py-4 border-b border-gray-100 last:border-b-0"
            >
              {item.charAt(0).toUpperCase() + item.slice(1)}
            </button>
          ))}
